...
//
class axi_sequence_item#(d_width = 16, a_width = 16) extends uvm_sequence_item;
    typedef axi_sequence_item#(d_width, a_width) this_type_t;
    `uvm_object_param_utils(axi_sequence_item#(d_width, a_width));
...
/*  Functions                                                                 */
/*----------------------------------------------------------------------------*/
function void axi_sequence_item::do_print(uvm_printer printer);
    ...
    begin
        string data_str;
        data_str = "";
        foreach (data[i,j]) data_str = {data_str, $sformatf("%02h ", data[i][j])};
        printer.print_generic("Data", "dynamic array", 8*2**b_size*(b_len+1), data_str);
    end
...
function string axi_sequence_item::convert2string();
    ...
    begin
        string rresp_str;
        rresp_str = "";
        foreach (r_resp[i]) rresp_str = {rresp_str, $sformatf("%0h ", r_resp[i])};
        s = {s, $sformatf("Read resp      :   %s\n", rresp_str)};
    end

    return s;
endfunction: convert2string

function void axi_sequence_item::do_copy(uvm_object rhs);
    this_type_t rhs_;
